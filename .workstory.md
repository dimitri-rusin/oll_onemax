...
__conda_setup="$('/opt/dev/Langs/Conda3-2023.02/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/opt/dev/Langs/Conda3-2023.02/etc/profile.d/conda.sh" ]; then
        . "/opt/dev/Langs/Conda3-2023.02/etc/profile.d/conda.sh"
    else
        export PATH="/opt/dev/Langs/Conda3-2023.02/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<

export PATH=$PATH:/scratchbeta/rusind/singularity/bin
```
...
mkdir -p /home/dimitri/code/oll_onemax/mesu/
